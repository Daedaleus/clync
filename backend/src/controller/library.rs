...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/library/{name}", get(detail_handler).put(update_handler).delete(delete_handler))
        .route("/library/{name}/thumbnail", post(upload_thumbnail_handler)
            .layer(DefaultBodyLimit::disable()))
        .route("/library/{name}/autofill", get(autofill_candidates_handler).post(autofill_confirm_handler))
}
...
async fn list_handler(
    ...
    Ok(Json(GameService::new(Arc::clone(&state.db)).list_all().await?))
}
...
async fn create_handler(
    ...
    GameService::new(Arc::clone(&state.db)).create(body, &user.keycloak_id).await?;
...
async fn update_handler(
    ...
    GameService::new(Arc::clone(&state.db)).update(&name, body).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn upload_thumbnail_handler(
        ...
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        ...
        GameService::new(Arc::clone(&state.db))
            .store_thumbnail(&name, bytes, content_type).await?;
...
async fn delete_handler(
    ...
    GameService::new(Arc::clone(&state.db)).delete_game(&name, user.is_admin).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn autofill_candidates_handler(
    ...
    let candidates = GameService::new(Arc::clone(&state.db))
        .get_autofill_candidates(&name, &state.rawg_api_key).await?;
    Ok(Json(candidates))
}
...
async fn autofill_confirm_handler(
    ...
    let game = GameService::new(Arc::clone(&state.db))
        .confirm_autofill(&name, body.rawg_id, &state.rawg_api_key, body.genre, &user.keycloak_id).await?;
    Ok(Json(game))
}
...
async fn serve_thumbnail_handler(
    ...
    match GameService::new(Arc::clone(&state.db)).get_thumbnail(&name).await {
