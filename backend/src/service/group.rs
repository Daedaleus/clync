...
impl GroupService {
    ...
    pub async fn create(&self, req: CreateGroupRequest, creator_id: &str) -> Result<GroupResponse, AppError> {
        let name = req.name.trim().to_owned();
        if name.is_empty() {
            return Err(AppError::Validation("Gruppenname darf nicht leer sein".into()));
        }
        let group = self
            .repo
    ...
    pub async fn get_detail(&self, group_id: &str, requester_id: &str) -> Result<Option<GroupDetailResponse>, AppError> {
        ...
        let (members_with_games, friend_ids) = tokio::try_join!(
            async {
                if group.members.is_empty() { Ok(vec![]) } else { self.repo.find_member_games(group.members).await }
            },
        ...
        let members = members_with_games.into_iter().map(|m| {
            let is_friend = friend_ids.contains(&m.keycloak_id);
            MemberResponse { keycloak_id: m.keycloak_id, username: m.username, is_friend }
        }).collect();
    ...
    pub async fn search_public(&self, query: &str, is_admin: bool) -> Result<Vec<GroupResponse>, AppError> {
    ...
    pub async fn my_groups(&self, keycloak_id: &str) -> Result<Vec<GroupSummary>, AppError> {
        Ok(self.repo.find_by_member(keycloak_id.to_owned()).await?.into_iter().map(Into::into).collect())
    }

    pub async fn join(&self, group_id: &str, keycloak_id: &str) -> Result<(), AppError> {
        self.repo.join(group_id.to_owned(), keycloak_id.to_owned()).await?;
        Ok(())
    }
...
pub(crate) fn possible_games(members: &[MemberWithGames]) -> Vec<PossibleGame> {
    ...
    let mut games: Vec<PossibleGame> = counts.into_iter()
...
mod tests {
    ...
    impl GroupRepo for PanicGroupRepo {
        async fn create(&self, _: String, _: bool, _: String) -> Result<Option<Group>, surrealdb::Error> { panic!("should not be called") }
        async fn find_by_id(&self, _: String) -> Result<Option<Group>, surrealdb::Error> { panic!() }
        async fn find_member_games(&self, _: Vec<String>) -> Result<Vec<MemberWithGames>, surrealdb::Error> { panic!() }
        async fn list_public(&self) -> Result<Vec<Group>, surrealdb::Error> { panic!() }
        async fn search_public(&self, _: String) -> Result<Vec<Group>, surrealdb::Error> { panic!() }
        async fn find_by_member(&self, _: String) -> Result<Vec<Group>, surrealdb::Error> { panic!() }
        async fn join(&self, _: String, _: String) -> Result<(), surrealdb::Error> { panic!() }
        async fn share_group(&self, _: String, _: String) -> Result<bool, surrealdb::Error> { panic!() }
        async fn delete(&self, _: String) -> Result<(), surrealdb::Error> { panic!() }
        async fn find_all(&self) -> Result<Vec<Group>, surrealdb::Error> { panic!() }
    }

    #[async_trait]
    impl UserRepo for PanicUserRepo {
        async fn upsert(&self, _: crate::model::user::User) -> Result<(), surrealdb::Error> { panic!() }
        async fn search(&self, _: String, _: String) -> Result<Vec<crate::model::user::UserSummary>, surrealdb::Error> { panic!() }
        async fn find_by_ids(&self, _: Vec<String>) -> Result<Vec<crate::model::user::UserSummary>, surrealdb::Error> { panic!() }
    ...
    async fn create_empty_name_returns_error() {
        let req = CreateGroupRequest { name: "   ".into(), is_public: true };
