...
impl SessionService {
    ...
    pub async fn create(&self, user: &AuthUser, req: CreateSessionRequest) -> Result<SessionResponse, AppError> {
        validate_create_request(&req)?;
        let game = req.game.trim().to_owned();
        let session = self.repo
            .create(user.keycloak_id.clone(), user.username.clone(), game, req.scheduled_at, req.scope, req.group_ids)
            .await?
    ...
    pub async fn get_feed(&self, user: &AuthUser) -> Result<Vec<SessionResponse>, AppError> {
        let my_group_ids = self.group_repo.find_by_member(user.keycloak_id.clone()).await?
            .into_iter().map(|g| g.id).collect();
        let sessions = self.repo.find_feed(my_group_ids).await?;
        Ok(sessions.into_iter().map(|s| s.into_response(&user.keycloak_id)).collect())
    }
    ...
    pub async fn get_for_group(&self, group_id: &str, requester_id: &str) -> Result<Vec<SessionResponse>, AppError> {
        let sessions = self.repo.find_for_group(group_id.to_owned()).await?;
        Ok(sessions.into_iter().map(|s| s.into_response(requester_id)).collect())
    }

    pub async fn get_detail(&self, session_id: &str, requester_id: &str) -> Result<Option<SessionDetailResponse>, AppError> {
        Ok(self.repo.find_by_id(session_id.to_owned()).await?
            .map(|s| s.into_detail_response(requester_id)))
    }

    /// Returns the updated session (for broadcasting), None if caller is the creator.
    pub async fn join(&self, session_id: &str, user: &AuthUser) -> Result<Option<SessionResponse>, AppError> {
        let session = self.repo.join(session_id.to_owned(), user.keycloak_id.clone()).await?;
        Ok(session.map(|s| s.into_response(&user.keycloak_id)))
    }

    pub async fn leave(&self, session_id: &str, user_id: &str) -> Result<(), AppError> {
        self.repo.leave(session_id.to_owned(), user_id.to_owned()).await?;
        Ok(())
    }

    /// Returns the group_ids for broadcast routing.
    pub async fn delete(&self, session_id: &str, user_id: &str, is_admin: bool) -> Result<Vec<String>, AppError> {
        if is_admin {
            Ok(self.repo.delete_as_admin(session_id.to_owned()).await?)
        } else {
            Ok(self.repo.delete(session_id.to_owned(), user_id.to_owned()).await?)
        }
    }
}
...
pub(crate) fn validate_create_request(req: &CreateSessionRequest) -> Result<(), AppError> {
    ...
    if scheduled <= chrono::Utc::now() {
        return Err(AppError::Validation("Session kann nicht in der Vergangenheit liegen".into()));
    }
    if req.scope != "global" && req.scope != "groups" {
        return Err(AppError::Validation("Ungültiger Scope (global oder groups)".into()));
    }
    if req.scope == "groups" && req.group_ids.is_empty() {
        return Err(AppError::Validation("Mindestens eine Gruppe auswählen".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn req(game: &str, scheduled_at: &str, scope: &str, group_ids: Vec<&str>) -> CreateSessionRequest {
    ...
    fn empty_game_name_is_rejected() {
        assert!(validate_create_request(&req("", "2026-05-20T18:00:00Z", "global", vec![])).is_err());
    }
    ...
    fn invalid_scope_is_rejected() {
        assert!(validate_create_request(&req("CS2", "2026-05-20T18:00:00Z", "public", vec![])).is_err());
    }

    #[test]
    fn groups_scope_without_group_ids_is_rejected() {
        assert!(validate_create_request(&req("CS2", "2026-05-20T18:00:00Z", "groups", vec![])).is_err());
    }

    #[test]
    fn global_scope_is_valid() {
        assert!(validate_create_request(&req("CS2", "2026-05-20T18:00:00Z", "global", vec![])).is_ok());
    }

    #[test]
    fn groups_scope_with_group_id_is_valid() {
        assert!(validate_create_request(&req("CS2", "2026-05-20T18:00:00Z", "groups", vec!["group1"])).is_ok());
    }
}
